...
d3.json("/data/albums.json").then(function (dataInput) {
  /*let data = [];

  for (album in dataInput) {
    const cd = new CD(
      dataInput[album].artistName,
      dataInput[album].albumName,
      dataInput[album].trackList.length
    );
    data.push(cd);
  }*/
  ...
  for (album in dataInput) {
    data.push(dataInput[album].trackList.length);
    //dataInput[album].numberOfTracks = dataInput[album].trackList.length;
  }
